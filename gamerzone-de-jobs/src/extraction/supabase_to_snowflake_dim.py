...
def create_snowflake_connection():
    ...
    return snowflake.connector.connect(
        ...
        session_parameters={
            "QUERY_TAG": "GAMERZONE_DIMENSION_INGESTION",
        },
    )
...
def extract_table(
    ...
    logger.info("Extracting %s from Supabase", table_name)
...
def main():
    ...
    # One ID for the entire dimension extraction run.
    ingestion_id = (
    ...
    logger.info("=" * 70)
    logger.info("Starting GamerZone dimension ingestion")
    ...
    try:
        ...
        logger.info("=" * 70)
        logger.info(
            "Dimension ingestion completed successfully"
        )
        logger.info(
            "Ingestion ID: %s",
            ingestion_id,
